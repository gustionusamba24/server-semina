...
const activateParticipant = async (req) => {
  ...
  if (!check)
    throw new NotFoundError("Participant does not registered");
...
const signinParticipant = async (req) => {
  ...
  if (result.status === "tidak aktif") {
    throw new UnauthorizedError(
      "Your account does not activated yet"
    );
  }
...
const checkoutOrder = async (req) => {
  ...
  if (!checkingPayment) {
    throw new NotFoundError(
      "There is no payment method with id :" + payment
    );
  }
    ...
    checkingEvent.tickets.forEach((ticket) => {
      if (tic.ticketCategories.type === ticket.type) {
        if (tic.sumTicket > ticket.stock) {
          throw new NotFoundError("Insufficient ticket stock");
        } else {
